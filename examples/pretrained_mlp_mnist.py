...
bchildren = [child for child in bmodel.model.mlp.children() if "Linear" in child.__class__.__name__]
for bchild in bchildren:
    print((bchild.weight.mu.data - bchild.weight.sample().data).sum().item())
exit()
...
for img, label in pbar:
    ...
    # Loss Computation
    prediction = bmodel(img.view(img.size(0), -1))
    loss = F.nll_loss(prediction, label, reduction="sum")
    acc = (torch.argmax(prediction, dim=1) == label).sum()
